...
int dfs2(char ochar, int r, int c, vector<vector<char>> a, unordered_set<pair<int, int>, pair_hash>& explored, unordered_set<pair<int, int>, pair_hash>& area) {
    pair<int, int> dirs[] = {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};
    pair<int, int> pos = make_pair(r, c);
    int res = 0;
    if (explored.find(pos) != explored.end() || !inBounds(r, c, a.size(), a[0].size())) {
...
int calculate_sides(char ochar, unordered_set<pair<int, int>, pair_hash>& perimeter, vector<vector<char>>& a) {
    int res = 0;
    for (auto p : perimeter) {
        int r = p.first;
        int c = p.second;
        
        bool left = !inBounds(r, c - 1, a.size(), a[0].size()) || a[r][c - 1] != ochar;
        bool right = !inBounds(r, c + 1, a.size(), a[0].size()) || a[r][c + 1] != ochar;
        bool up = !inBounds(r - 1, c, a.size(), a[0].size()) || a[r - 1][c] != ochar;
        bool down = !inBounds(r + 1, c, a.size(), a[0].size()) || a[r + 1][c] != ochar;

        bool upleft = !inBounds(r - 1, c - 1, a.size(), a[0].size()) || a[r - 1][c - 1] != ochar;
        bool upright = !inBounds(r - 1, c + 1, a.size(), a[0].size()) || a[r - 1][c + 1] != ochar;
        bool downleft = !inBounds(r + 1, c - 1, a.size(), a[0].size()) || a[r + 1][c - 1] != ochar;
        bool downright = !inBounds(r + 1, c + 1, a.size(), a[0].size()) || a[r + 1][c + 1] != ochar;
    ...
    for (int i = 0; i < a.size(); i++) {
        for (int j = 0; j < a[i].size(); j++) {
            ...
            if (explored.find(p) == explored.end()) {
                ...
                s += (dfs2(a[i][j], i, j, a, explored, area) * calculate_sides(a[i][j], area, a));
            }
        }
    }
    return s;
